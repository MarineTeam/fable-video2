...
import { normalizeEmail, isValidEmail, isAdmin } from './auth';
...
// Plans a whole-list membership change, PURE, so the caller can be told what
// happened to every address it named instead of just being handed the result.
//
// `approved` is the set of currently-approved viewers. An address outside it
// is REPORTED, not written: lib/viewerTags.js's addTagToViewers has always
// refused to tag a non-viewer ('Not an approved viewer'), and membership is
// the same kind of claim about the same kind of person. Writing one anyway
// would leave a group row for somebody with no account, which nothing cleans
// and which silently becomes real if that address is ever approved.
//
// Passing no `approved` set means 'do not check' — the caller could not load
// it. That fails toward today's behaviour rather than emptying a group
// because Redis blinked.
export function planMemberList({ current = [], wanted = [], approved = null } = {}) {
  const now = new Set((Array.isArray(current) ? current : []).map(normalizeEmail).filter(Boolean));
  ...
  for (const raw of Array.isArray(wanted) ? wanted : []) {
    ...
    // An address already in the group stays, even if the approved set no
    // longer holds it: removing somebody is an explicit act, not a side
    // effect of saving an unrelated edit.
    if (approved && !approved.has(email) && !now.has(email)) {
      unknown.push(email);
      continue;
    }
    named.push(email);
  ...
  const members = [...new Set(named)].sort();
  const keep = new Set(members);
  return {
    members,
    added: members.filter((email) => !now.has(email)),
    removed: [...now].filter((email) => !keep.has(email)).sort(),
    unknown: [...new Set(unknown)].sort(),
    invalid: [...new Set(invalid.filter(Boolean))].sort(),
  };
}
...
export async function setMembersOfGroup(groupId, emails, { approved = null } = {}) {
  const r = redis();
  const memberships = await loadGroupMemberships();
  const plan = planMemberList({
    current: membersOfGroup(memberships, groupId),
    wanted: emails,
    approved,
  });
  const wanted = new Set(plan.members);
  ...
  // The plan travels with the result so the admin sees who was added, who was
  // dropped, and — the one that matters — which addresses were refused for not
  // being approved viewers. A silent refusal is how somebody ends up believing
  // a person is in a group for months.
  return { ok: true, ...plan };
}
...
// reason around at best, and at worst a grant that a recycled id inherits —
// the same reasoning that already removes a deleted VIDEO from group scopes.
//
// Best-effort and last, like every other cleanup on the delete path: failing
// to tidy a scope must not turn a successful delete into an error.
export async function pruneCollectionFromGroups(collectionId) {
  const id = String(collectionId || '').trim();
  if (!id) return 0;
  try {
    const groupsById = await loadGroups();
    const touched = Object.values(groupsById).filter((g) => g.collectionIds.includes(id));
    await Promise.all(
      touched.map((group) =>
        saveGroup({ ...group, collectionIds: group.collectionIds.filter((c) => c !== id) })
      )
    );
...
// Adds one video to several groups' scopes — the upload form's "visible to"
// choice. Each group is written on its own, so one failing costs only that
// group, and the caller is told which did not take: the video exists by the
// time this runs. A group that vanished or filled since the route checked is
// reported as failed — saveGroup would otherwise recreate the first and
// silently drop a (sorted-last) grant from the second.
export async function grantVideoToGroups(guid, groupIds) {
  ...
  try {
    ...
    return { granted, failed: [...(groupIds || [])] };
  }
  for (const gid of groupIds || []) {
    ...
    try {
      ...
      if (!group.videoIds.includes(id)) {
        if (group.videoIds.length >= MAX_SCOPE_ENTRIES) throw new Error('Group is full');
        const result = await saveGroup({ ...group, videoIds: [...group.videoIds, id] });
...
// Called when a video is deleted — the video-level twin of
// pruneCollectionFromGroups. Uploads can now grant groups, and cancelling an
// upload deletes its half-made video, so without this every cancelled upload
// left its guid in the groups it was ticked into. Best-effort, like its twin.
export async function pruneVideoFromGroups(guids) {
  const gone = new Set((Array.isArray(guids) ? guids : [guids]).map(String).filter(Boolean));
  if (!gone.size) return 0;
  try {
    const groupsById = await loadGroups();
    const touched = Object.values(groupsById).filter((g) => g.videoIds.some((v) => gone.has(v)));
    await Promise.all(
      touched.map((group) =>
        saveGroup({ ...group, videoIds: group.videoIds.filter((v) => !gone.has(v)) })
      )
    );
    return touched.length;
  } catch {
...
//
// A staff member limited to certain groups (lib/staffScopeRules.js) is the
// exception: they see what their groups grant — never the whole library, and
// never more than that because a read failed.
export async function contentScopeFor(email, { staff } = {}) {
  if (!groupGatingEnabled()) {
    return { unrestricted: true, collectionIds: [], videoIds: [] };
  }
  if (staff) {
    if (isAdmin(email)) return { unrestricted: true, collectionIds: [], videoIds: [] };
    try {
      const scope = await scopeForEmail(email);
      if (!scope) return { unrestricted: true, collectionIds: [], videoIds: [] };
      return contentOfScope(scope, await loadGroups());
    } catch {
      return DENY_SCOPE;
    }
  }
