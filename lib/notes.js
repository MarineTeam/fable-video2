...
export function noteMatches(notes, query, passage = parsePassageQuery(query)) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return false;
  if (String(notes || '').toLowerCase().includes(q)) return true;
  return passage ? passageMatches(parseReferences(notes), passage) : false;
}
...
export function matchingNoteGuids(notesByGuid, query) {
  ...
  return Object.entries(notesByGuid || {})
    .filter(([, notes]) => noteMatches(notes, q, passage))
    .map(([guid]) => guid)
    .sort();
}
