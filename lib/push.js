...
// `toEmails` addresses a specific set (e.g. everyone holding a capability);
// omitted, it means every currently-allowed viewer, which is what the
// new-video announcement and the admin broadcast both want. Either way
// eligibleSubs still filters to currently-allowed addresses, so a removed
// viewer never receives a push.
export async function sendToAll(payload, { toEmails = null } = {}) {
  if (!pushEnabled()) return { sent: 0, pruned: 0 };
  if (toEmails && toEmails.length === 0) return { sent: 0, pruned: 0 };
  const r = redis();
  ...
  const targets = eligibleSubs(entries, toEmails || (await allowedEmailSet()));
