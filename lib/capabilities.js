...
// Property 3 has a blind spot the subset rule alone does not cover.
//
// lib/guard.js derives viewer approval from capability-holding
// (`viewerAccessFor`: `approved = staff`, staff = holds >= 1 capability), so
// assigning ANY role to a stranger also hands them the private video library.
// That outcome is what `viewers.manage` gates — but `canDelegate` sees only
// capabilities, and the actor genuinely holds the one they are handing out, so
// it passes. A roles.manage holder could therefore add viewers without holding
// viewers.manage.
//
// This closes it: giving a first foothold to someone who cannot already see the
// library additionally requires viewers.manage. Owners are unaffected, removing
// roles (`grantedCaps` empty) is unaffected, and re-assigning roles to someone
// already approved is unaffected — only the case that actually widens who can
// watch is refused.
...
export function emailsWithCapability({ owners = [], assignments = {}, rolesById = {}, cap } = {}) {
  ...
  for (const [email, roleIds] of Object.entries(assignments)) {
    ...
    if (caps.includes(cap)) out.add(email);
  }
  return [...out].sort();
}
