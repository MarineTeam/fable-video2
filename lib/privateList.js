...
// The whole index for one video, when the video itself is deleted. Only the
// index: the shares it tracked are records of their own, listed and revoked
// from the Shares tab, and a link to a deleted video already plays nothing.
export async function clearPrivateList(videoId) {
  await redis().del(privateListKey(videoId));
}
...
// not one per field).
export async function recordPrivateListShare(videoId, email, id, share) {
  const r = redis();
  const key = privateListKey(videoId);
  await r.hset(key, { [normalizeEmail(email)]: id });
  await r.set(shareKey(id), { ...share, viaPrivateList: true }, { ex: ttlSecondsFor(share.expiresAt) });
