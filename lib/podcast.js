...
export function buildFeedXml({ title, description, siteUrl, feedUrl, imageUrl, items = [] }) {
  const channel = [
  ...
  if (imageUrl) channel.push(`<itunes:image href="${escapeXml(imageUrl)}"/>`);
  ...
  const entries = items.map((item) => {
    ...
    if (item.imageUrl) parts.push(`<itunes:image href="${escapeXml(item.imageUrl)}"/>`);
