...
// Explicit sweep of the `shares` index for ids whose record is truly gone
// (past its grace window). The admin Shares list already does this as a
// side effect of every GET (see pages/api/admin/shares.js) — this is the
// same logic, exposed standalone so a manual "clean up" action can run it
// without needing the full list (titles, sorting, etc.) along for the ride.
export async function pruneGoneShares() {
  const r = redis();
  const ids = (await r.smembers(k('shares'))) || [];
  if (ids.length === 0) return 0;
  const loaded = await loadShares(ids);
  const gone = ids.filter((id, i) => !loaded[i]);
  if (gone.length > 0) await r.srem(k('shares'), ...gone).catch(() => {});
  return gone.length;
}
