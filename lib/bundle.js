...
export async function createOrExtendBundle({ email, newItemIds, sweepItemIds = [], expiresAt }) {
  ...
  await r.sadd(k('bundles'), bundle.id);
...
// Pure eligibility rule, kept separate from Redis so it's unit-testable:
// a bundle is stale once its record still exists but every member share has
// since expired or been revoked. `expiresAt` on the bundle only ever moves
// forward (never shrinks to match a departed member — see
// createOrExtendBundle/extendBundleExpiry), so an emptied-out bundle can sit
// around for a long time otherwise, waiting on a TTL that has nothing left
// to protect.
export function isBundleStale(bundle, liveItemCount) {
  return Boolean(bundle) && liveItemCount === 0;
}

// Reclaim bundles early instead of waiting out their TTL: sweep the
// `bundles` index (kept in lib/bundle.js's createOrExtendBundle) for ids
// whose record is already gone past its own grace window, and for ids whose
// record exists but has zero currently-active members left. One MGET for
// every indexed bundle id, then one loadShares MGET per bundle to count live
// members (title lookups are skipped — this only needs a count).
export async function cleanupStaleBundles() {
  const r = redis();
  const ids = (await r.smembers(k('bundles'))) || [];
  if (ids.length === 0) return { checked: 0, staleBundles: 0, goneBundles: 0 };
  const records = await r.mget(...ids.map((id) => k(`bundle:${id}`)));
  const gone = [];
  const stale = [];
  await Promise.all(
    ids.map(async (id, i) => {
      ...
      const bundle = { id, ...record };
      const shares = await loadShares(bundle.itemIds);
      const liveCount = shares.filter((share) => isShareActive(share)).length;
      if (isBundleStale(bundle, liveCount)) stale.push(bundle);
    })
  );
  await Promise.all(
    stale.map(async (bundle) => {
      // Only clear the email pointer if it still points at this exact
      // bundle — the recipient may already have a newer one in flight.
      const pointer = await r.get(k(`bundle-by-email:${bundle.email}`)).catch(() => null);
      if (pointer === bundle.id) {
        await r.del(k(`bundle-by-email:${bundle.email}`)).catch(() => {});
      }
      await r.del(k(`bundle:${bundle.id}`)).catch(() => {});
    })
  );
  const toRemoveFromIndex = [...gone, ...stale.map((b) => b.id)];
  if (toRemoveFromIndex.length > 0) {
    await r.srem(k('bundles'), ...toRemoveFromIndex).catch(() => {});
  }
  return { checked: ids.length, staleBundles: stale.length, goneBundles: gone.length };
}
