...
import { extendShare, isShareActive, ttlSecondsFor, GRACE_SECONDS } from './share';
...
// pre-existing, not-yet-bundled shares the first time one is created.
export async function activeSharesForEmail(email, excludeIds = []) {
  ...
  await Promise.all(
    ids.map(async (id) => {
      ...
      if (!share || normalizeEmail(share.email) !== norm || !isShareActive(share)) return;
      out.push({ id, ...share });
    })
  );
  return out;
...
async function tagShareBundle(id, bundleId) {
  const key = k(`share:${id}`);
  const r = redis();
  const share = await r.get(key).catch(() => null);
  if (!share) return;
  const ttl = await r.ttl(key).catch(() => -1);
  await r.set(key, { ...share, bundleId }, { ex: ttl > 0 ? ttl : GRACE_SECONDS });
}
...
export async function createOrExtendBundle({ email, newItemIds, sweepItemIds = [], expiresAt }) {
  ...
  await Promise.all([...new Set([...newItemIds, ...sweepItemIds])].map((sid) => tagShareBundle(sid, bundle.id)));
  return bundle;
}
...
export async function liveBundleItems(bundle, origin) {
  ...
  const items = [];
  await Promise.all(
    bundle.itemIds.map(async (id) => {
      const share = await r.get(k(`share:${id}`)).catch(() => null);
      if (!isShareActive(share)) return;
      let videoTitle = share.videoId;
      try {
        videoTitle = (await getVideo(share.videoId))?.title || share.videoId;
      } catch {}
      items.push({ id, url: `${origin}/s/${id}`, videoTitle, expiresAt: share.expiresAt });
    })
  );
