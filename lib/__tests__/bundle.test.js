...
import { decideBundleAction, isBundleStale } from '../bundle';
...
describe('isBundleStale', () => {
  it('is stale once every member has expired or been revoked', () => {
    expect(isBundleStale({ id: 'b1' }, 0)).toBe(true);
  });

  it('is not stale while at least one member is still active', () => {
    expect(isBundleStale({ id: 'b1' }, 1)).toBe(false);
  });

  it('is never stale for a bundle that does not exist', () => {
    expect(isBundleStale(null, 0)).toBe(false);
  });
});
