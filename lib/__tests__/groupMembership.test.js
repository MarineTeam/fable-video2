...
vi.mock('../guard', () => ({
  ...
  // The same gate, answering with the actor for routes that need it.
  requireActor: async (req, res, capability) => {
    if (!caps.includes(capability)) {
      res.status(403).json({ error: 'Forbidden' });
      return null;
    }
    return { email: 'admin@example.com', owner, capabilities: caps, staff: true, staffScope: null };
  },
}));
