...
describe('word stems in notes', () => {
  const notes = 'Jesus was baptized in the Jordan\nForgiveness and grace';

  it('finds another form of the same word', () => {
    expect(noteMatches(notes, 'baptism')).toBe(true);
    expect(noteMatches(notes, 'forgiving')).toBe(true);
  });

  it('needs every word, in any order', () => {
    expect(noteMatches(notes, 'grace baptise')).toBe(true);
    expect(noteMatches(notes, 'grace wilderness')).toBe(false);
  });

  it('flows through matchingNoteGuids', () => {
    expect(matchingNoteGuids({ a: notes, b: 'Harbour tour' }, 'baptising')).toEqual(['a']);
  });

  it('does not let stems widen a PASSAGE query to the whole book', () => {
    // Every word of 'Philippians 2' is present; the passage is not.
    expect(noteMatches('Part 2 — Philippians 4:10-20', 'Philippians 2')).toBe(false);
    expect(noteMatches('Part 2 — Philippians 4:10-20', 'Philippians')).toBe(true);
  });
});
