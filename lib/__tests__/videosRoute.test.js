...
vi.mock('../captionsStore', () => ({
  loadAllTranscriptText: async () => transcripts,
...
describe('searching translations', () => {
  it('finds a video by words said only in one of its translations', async () => {
    bunnyItems = [];
    translated = ['es1'];
    fetched.set('es1', { ...video(9), guid: 'es1' });
    const res = await call({ q: 'oveja perdida' });
    expect(translatedQuery).toBe('oveja perdida');
    expect(res.body.videos.map((v) => v.id || v.guid)).toEqual(['es1']);
  });

  it('counts translation matches against the same union cap', async () => {
    bunnyItems = [];
    translated = Array.from({ length: 30 }, (_, i) => `es${i}`);
    for (let i = 0; i < 30; i += 1) fetched.set(`es${i}`, { ...video(i), guid: `es${i}` });
    const res = await call({ q: 'oveja' });
    expect(res.body.matched).toBe(25);
    expect(res.body.truncated).toBe(true);
  });
});
