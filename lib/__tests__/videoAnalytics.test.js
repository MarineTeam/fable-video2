...
describe('rollupSharesByVideo', () => {
  ...
  it('captures the video title already attached by the shares API, falling back to the id', () => {
    const withTitle = rollupSharesByVideo([
      s({ videoTitle: '' }),
      s({ videoTitle: 'Reef Survey' }),
      s({ videoTitle: 'Ignored — first non-empty wins' }),
    ]);
    expect(withTitle.v1.videoTitle).toBe('Reef Survey');

    const noTitle = rollupSharesByVideo([s({})]);
    expect(noTitle.v1.videoTitle).toBe('v1');
  });
});
