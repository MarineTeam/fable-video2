...
const EVERYTHING = ['window', 'chapters', 'notes', 'transcript', 'public', 'rating', 'comments'];
...
describe('forgetVideo', () => {
  it('clears every kind of per-video data', async () => {
    ...
    expect(cleared.sort()).toEqual(EVERYTHING.map((n) => `${n}:vid-1`).sort());
  });

  it('carries on when one store fails', async () => {
    failing = 'notes';
    await expect(forgetVideo('vid-1')).resolves.toBeUndefined();
    expect(cleared).toHaveLength(EVERYTHING.length);
  });
