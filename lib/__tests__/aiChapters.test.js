...
vi.mock('../guard', () => ({
  ...
  requireActor: async (req, res) => {
    if (!admin) {
      res.status(403).json({ error: 'Not allowed' });
      return null;
    }
    return { email: admin, owner: true, capabilities: [], staff: true, staffScope: null };
  },
}));
...
describe('the collection marker', () => {
  it('is recorded when a paid job is queued', async () => {
    ...
    // click is mandatory again — the exact failure the marker removes.
    await call({ guid: GUID });
    expect(markTranscribePending).toHaveBeenCalledWith(GUID);
  });
  ...
  it('is cleared by a successful manual ingest', async () => {
    ...
    await call({ guid: GUID, ingest: true });
    expect(clearTranscribePending).toHaveBeenCalledWith(GUID);
  });

  it('survives a marker write that fails', async () => {
    // Best-effort by contract: the money is spent by that line, so losing the
    // marker must not be reported as a failed transcription.
    markTranscribePending.mockImplementationOnce(async () => {
      throw new Error('redis down');
    });
    expect((await call({ guid: GUID })).statusCode).toBe(200);
  });
});
