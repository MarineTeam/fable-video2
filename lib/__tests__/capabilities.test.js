...
describe('emailsWithCapability', () => {
  const rolesById = {
    stewards: { id: 'stewards', name: 'Stewards', capabilities: [CAP.VIEWERS_MANAGE] },
    auditor: { id: 'auditor', name: 'Auditor', capabilities: [CAP.AUDIT_READ] },
  };
  const assignments = {
    'steward@example.com': ['stewards'],
    'auditor@example.com': ['auditor'],
    'both@example.com': ['stewards', 'auditor'],
  };
  const owners = ['owner@example.com'];

  it('returns owners plus everyone whose roles include the capability', () => {
    expect(emailsWithCapability({ owners, assignments, rolesById, cap: CAP.VIEWERS_MANAGE })).toEqual([
      'both@example.com',
      'owner@example.com',
      'steward@example.com',
    ]);
  ...
  it('excludes assignees holding only other capabilities', () => {
    expect(
      emailsWithCapability({ owners: [], assignments, rolesById, cap: CAP.VIEWERS_MANAGE })
    ).toEqual(['both@example.com', 'steward@example.com']);
  });

  // Owners hold the whole catalog by definition, so they are always included
  // even when nobody has been assigned a role at all.
  it('still returns owners when there are no roles or assignments', () => {
    expect(emailsWithCapability({ owners, cap: CAP.VIEWERS_MANAGE })).toEqual(['owner@example.com']);
    expect(emailsWithCapability({ cap: CAP.VIEWERS_MANAGE })).toEqual([]);
  });

  it('never double-counts an owner who also holds a role', () => {
    expect(
      emailsWithCapability({
        owners: ['steward@example.com'],
        assignments,
        rolesById,
...
// The escalation the subset rule cannot see: lib/guard.js derives viewer
// approval from holding any capability, so a role assignment is also a grant of
// library access — which viewers.manage, not roles.manage, is supposed to gate.
