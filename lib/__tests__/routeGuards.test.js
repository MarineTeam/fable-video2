...
const CAPABILITY_ROUTES = {
  ...
  'admin/chapters': () => import('../../pages/api/admin/chapters'),
  ...
  'admin/notes': () => import('../../pages/api/admin/notes'),
  'admin/order': () => import('../../pages/api/admin/order'),
  'admin/private-list': () => import('../../pages/api/admin/private-list'),
  'admin/public-video': () => import('../../pages/api/admin/public-video'),
...
const VIEWER_ROUTES = {
  ...
  'feed-token': () => import('../../pages/api/feed-token'),
  'request-access': () => import('../../pages/api/request-access'),
  comments: () => import('../../pages/api/comments'),
};
...
// Scheduled jobs have no session at all — their caller is Vercel's cron
// runner — so the usual 401-for-anonymous rule is replaced by CRON_SECRET
// (lib/cronAuth.js). Pinned here with the other deny-by-default rows: without
// the secret header every method is refused, and with no secret configured
// the route does not exist.
describe('deny-by-default: scheduled-job routes refuse callers without CRON_SECRET', () => {
  const loader = () => import('../../pages/api/cron/transcripts');

  it('refuses every method from a caller without the secret', async () => {
    process.env.CRON_SECRET = 'a-configured-secret-0123456789';
    try {
      for (const method of METHODS) {
        const out = await callRoute(loader, method);
        expect(REFUSALS, `cron/transcripts answered ${out.statusCode} to ${method}`).toContain(out.statusCode);
      }
      expect((await callRoute(loader, 'GET')).statusCode).toBe(401);
    } finally {
      delete process.env.CRON_SECRET;
    }
  });

  it('does not exist when CRON_SECRET is not configured', async () => {
    delete process.env.CRON_SECRET;
    for (const method of METHODS) {
      expect((await callRoute(loader, method)).statusCode).toBe(404);
    }
  });
});
