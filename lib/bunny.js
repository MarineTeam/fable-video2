...
export function signedCdnUrl(path, ttlSeconds = 48 * 3600) {
  ...
  const expires = Math.floor(Date.now() / 1000) + ttlSeconds;
  // Not password hashing: bunny.net's CDN token-auth formula, fixed by their
  // verifier (see sha256Hex above for the same rationale). The suppression sits
  // on the .update() line because that is where code scanning anchors the alert
  // — the tainted env value flows in there, not at .createHash().
  const token = crypto
    .createHash('sha256')
    .update(`${key}${path}${expires}`) // codeql[js/insufficient-password-hash] -- not a password hash, see comment above
    .digest('base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=+$/, '');
