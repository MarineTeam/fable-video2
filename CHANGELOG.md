...
- The PWA manifest is now generated by `pages/api/manifest.js` but **keeps its
  ...
  and the route stays public and pre-login exactly as the static file was. Known
  consequence, stated rather than hidden: the service worker caches that URL, so
  an already-installed app keeps its old name until its cached manifest
  refreshes.
