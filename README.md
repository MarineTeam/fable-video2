...
| `GROUP_CONTENT_GATING` | Set to `1` to make **group content scopes actually restrict what members can see** (homepage list, collection filters, and direct `/watch/…` links alike). Unset → groups are membership bookkeeping only: scopes are recorded in `/admin` → Groups but change nobody's library. When it is on, a viewer in no group is governed by the live **"viewers in no group see"** setting in that tab (whole library by default, so turning the flag on never silently blanks anyone); a viewer in one or more groups sees exactly the union of those groups' collections and videos, and a group scoped to nothing grants nothing. Owners and role-holders always bypass it — unless a role-holder's roles are **limited to certain groups** (see Roles), who see what those groups see. Share links (`/s/…`, `/b/…`) are never group-gated. |
...
- **Roles** — capability-based access for people who are *not* in `ADMIN_EMAILS`. Create roles from a
  ...
  **Limit to certain groups** (needs `GROUP_CONTENT_GATING=1`): a person's roles can be limited to some
  groups. Their capabilities then reach only those groups, their members, and the videos those groups
  grant — in `/admin` and in the library, where they see what their groups see. They upload into their
  groups, delete only videos no other group can see, add new viewers straight into one of their groups
  (the membership is written before the approval), and change who is in their groups; they never create,
  re-scope or delete a group, move videos between collections, reorder the homepage, make a video public,
  or reach Settings, Roles, the activity log or broadcasts, which a limit strips. They can't take anyone
  out of their last group (while "viewers in no group see the whole library" is set, that would hand them
  everything) or remove someone who is also in another group. Owners can't be limited, and a limit whose
  groups were all deleted means *no* groups — never the whole portal. Stored in `fable2:user:scope`;
  the rules are `lib/staffScopeRules.js`.
