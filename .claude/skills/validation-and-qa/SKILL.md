...
`npm test` → vitest run → **23 files, 352 tests, all passing, ~2s**. Every
...
`viewerTags`, `watermark`, `schedule`, `accessRequests`, `siteName`, `chapters`, `notes`, `publicVideos`, `podcast` all lack rows). The two 2026-08-30 rows are appended
...
Re-verify the counts any time: `npm test` must report `Test Files 23 passed (23)`
and `Tests 352 passed (352)` (plus any you have since added).
...
`.github/workflows/ci.yml`; and by running `npm test` (23 files / 352 tests /
