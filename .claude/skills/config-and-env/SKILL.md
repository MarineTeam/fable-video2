...
|---|---|---|---|---|
| Site name | `fable2:settings:siteName` | /admin → Settings | GET/POST `/api/admin/settings` | Trimmed, whitespace-collapsed, ≤40 chars (`lib/siteName.js` `normalizeSiteName`). A blank submission DELETES the key — "reset to default" and "never set" are one state. Every read goes through `siteNameOrDefault`, so an unset or unreadable value renders `Marine Video Portal` rather than an empty header. Also drives the PWA manifest's `name`/`short_name` (`/api/manifest`) |
...
| Group default access | `fable2:settings:groupDefaultAccess` | /admin → Groups (only shown while gating is on) | PATCH `/api/admin/groups` action `set-default-access` | `'open'` \| `'closed'` — what a viewer in NO group sees while `GROUP_CONTENT_GATING=1`. Anything but `'closed'` reads as `'open'` (`lib/groups.js` `groupDefaultAccess`), and an unreadable setting also falls back to `'open'`, so turning the env flag on never silently blanks an audience. Note the asymmetry: this *default* fails open, while the scope resolution itself fails closed (`DENY_SCOPE`) — the first is a policy default, the second an authorization error |
| Roles & assignments | `fable2:roles` (HASH, roleId → role) + `fable2:user:roles` (HASH, email → roleId array) | /admin → Roles | GET/POST/PUT/PATCH/DELETE `/api/admin/roles` | Capability strings validated against the code catalog in `lib/capabilities.js` and dropped if unknown — Redis can never widen the catalog. Capped 50 roles, 10 roles/user, names 60 chars. **Owners (`ADMIN_EMAILS`) are never read from here**; every mutation is subject to the subset rule (`canDelegate`) |
| Groups & membership | `fable2:groups` (HASH, groupId → group) + `fable2:user:groups` (HASH, email → groupId array) | /admin → Groups | GET/POST/PUT/PATCH/DELETE `/api/admin/groups` | Names 60 chars whitespace-collapsed; capped 100 groups, 20 groups/user, 500 scope entries per list. Membership editable per-group or per-user; removing a viewer clears their memberships |
| Viewer tags/groups | `fable2:viewer:tags` (HASH, email → tag array) | /admin → Viewers | GET `/api/admin/viewers` (returns tags per viewer); POST `/api/admin/viewers-bulk` (`add-tag`/`remove-tag` over a selection) | Tag trimmed, whitespace-collapsed, capped 40 chars (`lib/viewerTags.js` `normalizeTag`); capped 20 tags/viewer; only current members of `viewers` can be tagged; removing a viewer outright clears their tags too |
