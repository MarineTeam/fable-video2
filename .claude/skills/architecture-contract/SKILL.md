...
| I3c | A video's publish window is enforced at the list AND at the direct URL, or it is not enforced | `pages/api/videos.js` (`filterVideosBySchedule`) and `pages/watch/[id].js` GSSP (`isWithinWindow`), staff bypassing both | Filtering the homepage while `/watch/<guid>` still plays is theatre — and worse, it looks like it works |
| I3d | The two UNAUTHENTICATED surfaces deny by default and leak nothing about the library | `lib/publicWatch.js` (public flag required, store fails closed, publish window applies, identical `notFound` for every refusal) and `pages/api/feed/[token].js` (token resolves to one viewer, that viewer's CURRENT access re-checked per fetch, items through the same scope+window pipeline, identical bare 404 for every refusal, `no-store`). **Verified signed out 2026-09-13**: both return 404 from their own code while `/`, `/admin`, `/watch/<guid>` and `/activity` 307 to `/auth/login` — so no `middleware.js` matcher change was needed | An anonymous visitor could enumerate the library, or a revoked viewer keep a working feed |
...
| I13 | Per-viewer data is keyed by the VIEWER, and aggregates derived from it hold no identity (**new 2026-09-19**) | `progress:{email}`, `mylist:{email}`, `ratings:{email}` — anything recorded about a person carries their email in the KEY, never as a field under the thing it is about, so removing a viewer is already the whole deletion story. `rating_counts` holds `guid:up`/`guid:down` integers and no address; it is maintained by `HINCRBY` of a delta (`lib/ratings.js` `voteDelta`) **after** the vote is written and outside its error path (`pages/api/rating.js`), so a counter failure can never report a successful vote as failed. `clearVideoRatingCounts` on video delete. Verify: `npm test -- ratings ratingRoute` | A hash keyed by video with the email as a field reads better and makes totals one HGETALL — and leaves a removed viewer's address in a row nothing cleans, since there is no sweep over video keys |
...
| `transcribe_pending` | HASH | videoGuid → epoch ms queued (**new 2026-09-20**) | HSET `markTranscribePending` on a successful queue (`pages/api/admin/transcribe.js`) | HGETALL → `planCollection` (`lib/transcribeQueue.js`), read by the admin video list | cleared on ingest (manual or automatic) and when older than `PENDING_MAX_AGE_MS` (24h). A MARKER, not a job — nothing runs it; the collection happens on a request an admin was making anyway |
| `ratings:{email}` | HASH | videoGuid → `'up'` / `'down'` (**new 2026-09-19**) | HSET/HDEL `lib/ratingsStore.js` via `pages/api/rating.js` | HGETALL `getRatings` → `ratingOf` (`lib/ratings.js`) — the watch page's 👍/👎, server-rendered | none. Per-viewer, keyed by email like `progress:`/`mylist:`, **so removing a viewer takes their votes with them** — the reason the vote is not stored under the video (I13) |
| `rating_counts` | HASH | `{guid}:up` / `{guid}:down` → integer (**new 2026-09-19**) | HINCRBY of a delta, `applyRatingCounts`, after the vote and best-effort | HGETALL `getRatingCounts` → `countsByVideo` — the admin Videos tab badge, **staff only** | `clearVideoRatingCounts` on delete, so a recycled guid cannot inherit a score. Holds **no identities**. Can drift by one and is never recomputed (FEATURES.md); a negative reads as 0 |
...
| `notes` | HASH | videoGuid → string (**new 2026-09-13**) | HSET/HDEL `lib/notesStore.js` `setVideoNotes` via `pages/api/admin/notes.js`; HDEL on video delete | HGETALL `loadAllNotes` (admin list, `/api/videos` search widening, feed item descriptions), HGET `getVideoNotes` | none; ≤4000 chars, control/zero-width stripped, tabs and newlines kept. Empty DELETES the key |
| `public-videos` | SET | video guids explicitly made world-readable (**new 2026-09-13**) | SADD/SREM `lib/publicVideosStore.js` `setVideoPublic` via `pages/api/admin/public-video.js`; SREM `clearVideoPublic` on video delete | SISMEMBER `isVideoPublic` (`lib/publicWatch.js`), SMEMBERS `loadPublicVideoGuids` (admin list) | none. **Reads FAIL CLOSED** — the only store here that does, because it IS the gate between an anonymous visitor and a video rather than decoration over content already behind the viewer gate |
| `feed-token:<token>` | string | normalized email (**new 2026-09-13**) | SET `lib/podcastStore.js` `issueFeedToken`; DEL on regenerate and on `revokeFeedToken` | GET `emailForFeedToken` (`/api/feed/[token]`) | none — a feed URL that expired on its own would silently stop a podcast app. Revocation is explicit. Fails CLOSED: this resolves identity for an UNAUTHENTICATED request |
| `feed-token-by-email:<email>` | string | that viewer's current token (**new 2026-09-13**) | SET `issueFeedToken`; DEL `revokeFeedToken`, and on viewer removal (`pages/api/admin/viewers.js` DELETE) | GET `getFeedToken` (viewer UI) | none; one token per viewer, so regenerating cannot leave two live URLs for one person |
| `settings:siteName` | string | the portal's display name (**new 2026-09-03**) | SET/DEL `lib/siteNameStore.js` `setSiteName` via `pages/api/admin/settings.js` POST (`settings.manage`) | GET `getSiteName` — via `withSiteName`, which injects it into every prop-returning page result, and via `pages/api/manifest.js` | none. Normalized to ≤40 chars, whitespace-collapsed; a blank value DELETES the key rather than storing an empty string, so "reset to default" and "never set" are the same state. Reads fall back to `DEFAULT_SITE_NAME` — branding is not an access decision, and a blank header (or a failed render) would be a worse failure than the stock name for one request |
...
| `share:<id>` | string (JSON object) | `{videoId, email, createdAt, expiresAt, views?, viewedAt?, lastViewedAt?, plays?, furthestPercent?, completedAt?, revokedAt?, bundleId?, viaPrivateList?}` (**updated 2026-07-25**: added `viaPrivateList`) | SET EX `lib/share.js` `createShare`; rewritten preserving TTL on every view `pages/s/[id].js` (views/lastViewedAt), on playback events `pages/api/share-event.js` (plays/furthestPercent/completedAt), on revoke `lib/share.js` `revokeShare` (soft-delete, sets `revokedAt`, no longer `DEL`), on extend `lib/share.js` `extendShare` (new `expiresAt`), on bundling `lib/bundle.js` `tagShareBundle` (`bundleId`), on private-list creation `lib/privateList.js` `recordPrivateListShare` (sets `viaPrivateList: true`, purely informational — never read for access-control or matching logic, only surfaced as a Shares tab badge) | `pages/s/[id].js`; `pages/api/admin/shares.js` (list, with computed `status`); `pages/api/admin/share.js` (resend/extend); `pages/api/share-event.js`; `lib/bundle.js` (`activeSharesForEmail`, `liveBundleItems`) | `EX` = `ttlSecondsFor(expiresAt)` = seconds-until-`expiresAt` **plus a 60-day grace window** (`lib/share.js` `GRACE_SECONDS`) — **not** simply `ttlHours×3600` as before 2026-07-21. Access control is decided by comparing the `expiresAt` field to now (`isShareActive`), not by whether the Redis record still exists — the grace window exists solely so an expired-but-not-revoked link can still be **extended**. `hours` still clamped 1–720h at creation/extension |
...
| `bundles` | SET | bundle ids (index over `bundle:<id>`, mirrors the `shares`/`share:<id>` pattern) | SADD `lib/bundle.js` `createOrExtendBundle` (**new 2026-07-23**) | SMEMBERS `lib/bundle.js` `cleanupStaleBundles` (via `pages/api/admin/cleanup.js`, the admin "Clean up stale items" action) | none; self-pruned by `cleanupStaleBundles` for ids whose record is gone or whose bundle has zero live members left (a bundle's `expiresAt` only ever moves forward, so an emptied bundle can otherwise sit well past the point its last member expired) |
...
1. **`email_verified` enforcement is implemented but OFF by default**
   (**changed 2026-08-31** — was "not checked at all"). `lib/auth.js`
   `trustedEmail` denies any session whose claim is not boolean `true`, wired
   at all 8 trust sites (I1b), but only when `REQUIRE_EMAIL_VERIFIED=1`. It is
   off by default because nothing in this repo can prove a given Auth0 tenant
   emits the claim, and enabling it blind locks out every user including every
   owner — with no admin UI left to fix it. **Two things are therefore still
   unproven and must be done on a live preview before turning it on:** the
   campaign's Phase 1 (confirm the claim is a real boolean via `/auth/profile`)
   and Phase 4 (log in as an unverified user and observe a designed denial, not
   a 500 or a redirect loop). Until the var is on, the operational mitigation —
   Auth0 sign-ups disabled tenant-wide — is still the only thing closing the
   hole.
...
10. **Legacy `pvp` names in client storage** (cosmetic). localStorage theme key
   ...
   (`public/sw.js:7`, now `pvp-static-v2`). Renaming them invalidates users' cached theme/assets
