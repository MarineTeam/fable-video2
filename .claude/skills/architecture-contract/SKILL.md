...
| I8 | The service worker caches only the 5 public static asset URLs — never authed pages, API responses, or video. (**2026-09-03**: `/manifest.webmanifest` is now generated by `pages/api/manifest.js` behind a rewrite, but the URL is unchanged, so the allowlist and this invariant are untouched. Consequence: an installed app keeps its old name until that cached URL refreshes.) | `public/sw.js:5-11` (allowlist) and `public/sw.js:31-38` (fetch handler responds only for allowlisted same-origin paths; everything else goes to network) | Protected content persists in cache storage on shared machines after logout; stale API data served offline |
