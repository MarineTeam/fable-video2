...
- **Private list** _(admin, Videos tab)_ — a **"Private list"** button on each video opens a persistent, editable invite for that one video, YouTube/Drive-style: the current recipients (every currently-active share for that video) are listed with a one-click **remove**, and an add form takes any number of emails at once. Adding is idempotent per email — anyone already on the list is left completely untouched (no duplicate share, no re-sent notification); only genuinely new emails get a fresh share created (at the maximum 720h/30-day expiry, so the invite reads as durable rather than needing a per-invite expiry decision) and, if requested, an email. Removing an email **revokes its share immediately** — the same soft-delete `lib/share.js` already uses for individual/bulk revoke — and re-inviting that email later creates a brand-new share, not a resurrection of the old one. A **"Notify new people by email"** checkbox, checked by default (matching Google Drive/YouTube's own share dialogs), controls only the notification: unchecked, every newly added email still gets a fully live, playable share, it just skips the send. The list itself is not a new stored entity — it's read directly from the same shares the Shares tab and per-video analytics already load, filtered to that video and to `status === 'active'` (`lib/privateList.js`), so it can never drift from what Revoke/Extend/the Shares tab show.
